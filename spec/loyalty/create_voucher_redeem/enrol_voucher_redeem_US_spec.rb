...
describe "enrol_loyalty_and_redeem" do
  ...
  it "should be able to get the welcome and birthday voucher" do
    ...
    loyalty_vochers = json_obj.collect {|x| x["code"]}
    expect(loyalty_vochers.count).to eq 2
  end

  it "should be able to bulk present the welcome and birthday voucher" do
    json_obj = Shim.bulk_present_vouchers(loyalty_vochers)
    expect(json_obj[0]["code"]).to include("#{loyalty_vochers[0]}")
    expect(json_obj[1]["code"]).to include("#{loyalty_vochers[1]}")
  ...
  it "should be able to bulk unpresent the welcome and birthday voucher" do
    json_obj = Shim.bulk_unpresent_vouchers(loyalty_vochers)
    expect(json_obj[0]["code"]).to include("#{loyalty_vochers[0]}")
    expect(json_obj[1]["code"]).to include("#{loyalty_vochers[1]}")
  ...
  it "should be able to bulk redeem vouchers" do
    json_obj = Shim.bulk_redeem_vouchers(loyalty_vochers)
    expect(json_obj[0]["code"]).to include("#{loyalty_vochers[0]}")
    expect(json_obj[1]["code"]).to include("#{loyalty_vochers[1]}")
